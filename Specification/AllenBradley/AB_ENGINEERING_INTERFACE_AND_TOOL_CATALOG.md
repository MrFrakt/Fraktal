...
| `fraktal_ab_gateway.py` | None (refuses writes) | Serves the projection to the generic HMI over the `fraktal.opcua.gateway.v1` WebSocket protocol (`websockets`, pinned in `requirements-gateway.txt`), so a live AB controller renders with no AB screens or repository. Read-only per §11.2.1: configured with no write root, it refuses `write`/`writeBatch` before the controller; binds loopback only; checks the Origin; holds the discovery revision stable and rejects a stale one. Reuses the projection's shared `read_document`, pins the serial, and re-validates the manifest hash every poll. Proved live 2026-09-21 through the HMI's production code path; evidence `Evidence/AB_HMI_GATEWAY_2026-09-21.md`. |
