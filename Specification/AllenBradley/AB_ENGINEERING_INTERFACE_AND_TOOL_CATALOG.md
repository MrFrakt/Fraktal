...
| `fraktal_ab_access_audit.py` | None | Offline External Access allow-list audit for AB §11.2.1: declared mailbox tags shall be `Read/Write`, declared public tags `Read Only`, everything else `None`. Nothing is inferred from a name and an omitted attribute is not treated as `None`. Three declared classes, not two: `--mailbox` (Read/Write), `--public` (Read Only) and `--plant` (Read/Write, but *not* a command surface). The plant class exists because a demo has no I/O card; declaring simulated sensors as mailboxes to make the audit pass would print `AirOk` in a report's mailbox list and tell the next reader it commands the machine. `CommandSurface` is reported apart from `WriteSurface` for the same reason. |
...
| `fraktal_ab_s16_fixture.py` | None | Generates the disposable S16 command-handshake fixture — one `DINT`-only context UDT, a module AOI carrying Core §6.1 over a simulated plant, and a mode owner with an AUTO chain and a MANUAL behavior. Inhibits embedded I/O, disables task output updates, rejects physical-I/O operands, and is scope-fenced by test against growing runtime-base structure. |
| `fraktal_ab_s16_execute.py` | Fixed writes | Exact S16 fingerprint, serial and arm guard; writes only `FRK_S16_Command`, `FRK_S16_Abort`, `FRK_S16_ModeSelect`, `FRK_S16_HoldRequest` and `FRK_S16_FaultRequest`, and restores all five in a `finally` block. Nine phases covering the handshake, abort, Execute-drop, held, fault/restart, mode switch and ordering/latency. Fails closed. **Not yet run against a controller.** |
| `fraktal_ab_declaration.py` | None | The committed declaration an application is emitted from, and the rules that refuse a bad one: an unnamed held reason, a timeout that is not a whole number of task scans, a ParCfg record not leading with `SchemaVersion`, a condition on an undeclared input, a transition to a step that does not exist, or a rendition list that omits ST. |
| `fraktal_ab_generate.py` | None | Turns one declaration into contract UDTs, one module AOI per declared type, the mode owner, the chain renditions and the full-project L5X. Asserts at emit time that the task carries the declared period the millisecond timeouts were converted from, that no public UDT carries a `BOOL`, and that nothing emitted names a physical-I/O operand. |
| `fraktal_ab_manifest.py` | None | Emits the controller-resident manifest from the same declaration, so a client discovers the station from the controller rather than from an L5X. Describes the declared graph once, rendition-agnostic - the field list comes from `publishable_tags`, so the rendition selector cannot reach the published contract. Sizes the key string to the declared maximum and publishes that width as `KeyLength`; a key that does not fit fails the build rather than truncating into a name it would share. Emits ASCII strings quoted and `$`-escaped, which is the only form Logix actually stores. |
| `fraktal_ab_manifest_read.py` | None | Reads the manifest off a controller and compares every declared row against the declaration it was generated from. Read-only by construction - no write path exists in the file - and the serial guard is required, not optional. Coherence is S7's protocol unchanged: read the revision, read every table, read it again, accept only if it did not move. Array tags are read with an explicit element count, because an array read without one returns row zero and *succeeds*. |
| `fraktal_ab_projection.py` | None | Projects a controller into the flat, transport-neutral snapshot document the HMI's OPC UA mapper consumes, so the generic HMI drives an AB controller with no AB-specific screens or repository. Fail-closed per Core 3.10: an invalid, truncated or disagreeing manifest refuses the whole projection. Unpublished surface is enumerated in `absent` with reasons, because the mapper defaults a missing key and silence would render as data. |
| `fraktal_ab_gateway.py` | Read default; §14 write gate | Serves the projection to the generic HMI over the `fraktal.opcua.gateway.v1` WebSocket protocol (`websockets`, pinned in `requirements-gateway.txt`), so a live AB controller renders with no AB screens or repository. Read-only by default per §11.2.1: with no write token it refuses `write`/`writeBatch` before the controller; binds loopback only; checks the Origin; holds the discovery revision stable and rejects a stale one. Reuses the projection's shared `read_document`, pins the serial, re-validates the manifest hash every poll. Proved live 2026-09-21 through the HMI's production code path (`Evidence/AB_HMI_GATEWAY_2026-09-21.md`). Writes were authorized 2026-09-21: `--write-token` arms the Core §14 gate (bearer-authenticated, no anonymous write, `HmiRequest` mailbox allow-list, monotonic `Sequence` commit), proved offline; an in-scope write is refused as "not connected" until the AB command binding exists (`Evidence/AB_HMI_GATEWAY_WRITE_ENABLE_2026-09-21.md`). |
| `fraktal_ab_mailbox_execute.py` | Mailbox commits (armed) | The command mailbox's evidence harness. Speaks the HMI's `fraktal.opcua.gateway.v1` protocol to a running gateway, commits a request the way `opcua_repository.dart` does (nine arguments, then `Sequence` as the commit marker), and proves each command by polling the controller's own `AckSequence` rather than trusting a write that returned true. Drives the six routed kinds, the three named refusals, and the anonymous / off-mailbox / replayed-sequence gate negatives. Refuses unless the controller publishes the expected ContentHash; armed explicitly. Proved live 2026-09-22; evidence `Evidence/AB_MAILBOX_COMMAND_2026-09-22.md`. |
| `fraktal_ab_mailbox_probe.py` | Mailbox arguments (armed) | Measures whether pylogix can write a user `StringFamily` member through `LEN`/`DATA` — the assumption the command-mailbox handover flagged as reasoned rather than measured. Writes arguments only and never `Sequence`, so it cannot commit a request, and restores what it touched. Answer: it works, for both the length-only and length-plus-data forms. A v33 type-map fact belonging with the S12 findings, and the counterpart to ST string-literal assignment, which v33 rejects. |
| `fraktal_ab_press_demo.py` | None | The press demo declaration: three module types, MANUAL jog, HOME, and an AUTO chain rendered in ST, native SFC and ladder. Simulated plant in tags; no control power, no physical I/O. |
| `fraktal_ab_rendition_gate.py` | None | Reads every emitted rendition back out of the L5X and recovers its step and transition sets in that rendition's own language - a `CASE` for ST, `EQU` rung-ins and `MOV`s for ladder, steps and directed links for a chart - then requires all of them to equal the declaration. A rendition that cannot be parsed back fails: silence is not parity. |
| `fraktal_ab_press_execute.py` | Fixed writes | Exact press-demo fingerprint, serial and arm guard; writes only the declared input tags and restores every one in a `finally` block. Fifteen rows covering the cycle, the held condition, first-out adoption, a reported-not-adopted child, the operator decision, MANUAL, HOME, mode switch, abort and the §3.13 chart marks. Reads each structure in one request. |
| `fraktal_ab_press_parity.py` | Fixed writes | Walks the AUTO graph on the bench in each rendition and requires identical traces, held behaviour and abort behaviour. Closes its measurement window with the machine rather than the observer, so a parity claim cannot depend on which language ran faster. |
