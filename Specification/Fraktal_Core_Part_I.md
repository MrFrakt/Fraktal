...
- **One catalog, generated from the enum.** The code→message catalog — with `{source}` / `{value}` placeholders for localization — **shall** be generated from the `E_Reason` definition so code and catalog cannot drift. The HMI localizes by `ErrorID`/`ReasonCode`; absent an entry it falls back to `Description`.
...
- **Rationalization record.** Each `E_Reason` **shall** carry, in the catalog generated from the enum (§8.8): a **priority** (derived from consequence × required response time, not assigned ad hoc), a one-line **operator action** ("what do I do about this?"), an **alarm class** (PROCESS / SAFETY / SYSTEM, from `ST_Diagnostic.Category`), and a flag for whether it is **suppressible/shelvable**. An entry without an operator action is an **event**, not an alarm, and **shall** be logged (§8.3) rather than annunciated — this is the primary lever against alarm overload.
...
### 8.10 Alarm performance & governance

Rationalization is set once; **performance** is monitored continuously.

- **Flood & nuisance handling.** Beyond the single-High strategy (§8.4), the Station Monitor **shall** debounce chattering conditions (a minimum on-time / fleeting filter per reason) and **shall** detect alarm **floods** (rate over a threshold in a rolling window), annunciating the flood condition itself rather than every member.
...
- **Performance KPIs.** The Station Monitor (§8.5) **shall** expose alarm-rate (per 10 min), standing-alarm count, and **stale**-alarm count (active beyond a long threshold) as OPC UA nodes, so alarm health is measurable against ISA-18.2 targets.
...
- The Unit **shall** maintain `GoodCount`, `NokCount`, and `ReworkCount`, incremented at `EVENT_PART_PROCESSED` (§3.16) from the part `Verdict`.
- A NOK **shall** be attributed by its first-out `ReasonCode` (§8.8) — the same reason vocabulary as a fault — so scrap is analysable by cause without a separate reject-code scheme. Counters reset on a deliberate, logged action (shift/changeover), never silently.
...
**(f) Time classification — the real cycle time (shall).** Every profiled step carries a **time class** (`E_TimeClass`): `WORK` — the default — or a wait class: `WAIT_UPSTREAM` (starved: awaiting the next pallet/part/material), `WAIT_DOWNSTREAM` (blocked: awaiting outfeed/downstream take-over), `WAIT_OPERATOR` (a §6.11 decision or manual intervention), `WAIT_EXTERNAL` (host/MES/tool response, §3.15/§11.6). A step declares its class **in the step record it already has** — one enum argument on the *wait* steps only (`_M_SetStep(…, Class := E_TimeClass.WAIT_UPSTREAM)`); every undeclared step is `WORK`, so the common case costs nothing (O1). The profiler accumulates per-class totals (`ByClass`) and publishes **`WorkTime` — the real cycle time** — and `WaitTime` alongside `Total`, so comparing against the design cycle time and OEE performance accounting use work-only time *by definition*, not by after-the-fact estimation (O3). The wait classes are the **per-step attribution of §8.11.3's Starved/Blocked**: `WAIT_UPSTREAM`/`WAIT_DOWNSTREAM` totals reconcile with the Unit's Starved/Blocked accumulators, and the Unit base **derives** its live `Starved`/`Blocked` status from the current step's wait class — one declaration, two views that cannot disagree (`FB_UnitBase`). A wait class **shall not** be used to hide process slowness: only genuine external waits qualify — that is what keeps `WorkTime` honest.
...
*Cross-references: §3.6 (logical HAL), §3.10 (self-description), §3.13 (HMI trees), §4.8 (browse paths), §8.6/§8.8 (system alarms & reasons), §7.6/§7.7 (forcing is gated), TC3 §10.6 (EtherCAT/ADS binding).*
...
| `CHANGEOVER_STARTED` / `CHANGEOVER_DONE` | Unit `CHANGEOVER` mode (§3.4) | work-order / product-definition change |
...
- Host events **shall** be exposed through a documented OPC UA object/method on the cell's server (§11.1), and **may** additionally be delivered over a socket/REST or MQTT-Sparkplug north-bound profile (§11.9) behind a provider, mirroring the recipe-provider injection (§3.8/§11.4) so transport is configuration, not module code.
