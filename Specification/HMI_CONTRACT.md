...
save/load/list only when the session permits `CONFIG_SET` (a model set also
requires `CHANGEOVER`) and the owning root is `READY`; a load is presented as
replacing the machine's parametrization, not as an edit. Because the PLC stages
and validates the whole set before applying any of it, a rejection returns the
offending `(Scope, WriteKey)` and the HMI shall show that identity rather than a
generic failure — the operator's next action is to fix one value, and they cannot
if the client hides which. Durability is displayed, never assumed: `PersistPending`
and `PersistFailed` from the root drive an explicit indication, and a restore
failure reported at startup stays visible until acknowledged. The client never
serializes or repairs a set itself; it names one and reports what the PLC did.
