...
| AB §11.2.1 | S8 | OPEN — posture decided, partly evidenced | CIP Security on v37+ recommended, zone/conduit the supported legacy posture, initial claim read-only so Core §14's write rule is armed by enabling a write root rather than relaxed. Measured: the Phase 0 controller implements **none** of CIP objects `0x5D`/`0x5E`/`0x5F`, and the required allow-list audit now exists and runs; [evidence](AllenBradley/Evidence/AB_S8_SECURITY_EVIDENCE.md). The legacy leg is now discharged: zones `Z-CTRL`/`Z-ENG`, conduits `C-EIP`/`C-ENG`, and a declared **SL-T 1 / SL-C 0** for the v33 bench, with SL 2 named as unreachable on this family rather than exceptable; [declaration](AllenBradley/Evidence/AB_S8_S9_REFERENCE_STATION_DECLARATIONS_2026-09-06.md). Still owed: a demonstrated CIP Security configuration on capable hardware, and the allow-list audit against a generated production L5X | writes cannot meet §14; binding stays read-only until fixed |
| AB §3.13, §7.7, §11.2 | S9 | OPEN — design decided, coherence proved | tiers are TC3's, freshness/poll budgets are declared per deployment, parity means the shared repository contract suite rather than an A/B rig, the mailbox payload is bounded to one unfragmented write, crash testing is scoped to the five replay-capable boundaries, redundancy is out of scope. Measured: the retry-until-stable guard never accepted a torn snapshot at any mutation rate, never falsely rejected a quiet controller, and converges whenever the mutation interval exceeds the guarded read window; [evidence](AllenBradley/Evidence/AB_S9_COHERENCE_EVIDENCE.md). The reference station is now declared against those costs: 4000-byte connection, live/fast 100 ms · slow 1000 ms · on-demand, three-missed-poll freshness, four reader sessions, and an explicit ~62 ms manifest-mutation limit for guard convergence; [declaration](AllenBradley/Evidence/AB_S8_S9_REFERENCE_STATION_DECLARATIONS_2026-09-06.md). Reconnect, quality and timestamp are now measured read-only: six independent sessions all served a correct first read in 4.4-7.5 ms against a 20.4 ms worst-case session round trip, so reconnect is budgeted at one poll period and faulted at three; the controller separates `Path segment error` (symbol absent, rediscover) from `Path destination unknown` (path invalid, do not retry) and fails both faster than it serves a good read; and with PTP disabled and `is_synchronized` false, every value carries the gateway read time plus `TimeSynchronized = FALSE`, so cross-controller ordering is not claimed; [evidence](AllenBradley/Evidence/AB_S9_RECONNECT_QUALITY_TIMESTAMP_2026-09-06.md). Still owed: the `WallClockTime` read behind the S1 fixture guard, the contract suite, and the mailbox matrix when writes are enabled | redesign snapshot/mailbox/gateway; no writable claim until the contract suite passes |
...
| Core §6.1, §6.2; AB §3.14 | S16 | **PASS** | the Core §6.1 PLCopen handshake and the §6.2 mode chain execute on Logix through generated composition, with no runtime base structure invented: the whole contract fits one `DINT`-only context UDT and two AOIs — module plus mode owner — using only types S12 proved, and the ordering rule is self-checked in the fixture rather than documented; [declaration](AllenBradley/Evidence/AB_S16_COMMAND_HANDSHAKE_DECLARATION_2026-09-06.md). Executed on the v33 bench: SDK import `0/0`, Studio v33 Verify `0/0`, canonical round trip identical, census clean, and all nine matrix phases passing identically on three consecutive runs — a held command is BUSY at LOW severity with reason 6101 and no `Error`, a fault raises `Error` with `ErrorID` 6102 at HIGH severity in ERROR state, abort never self-resumes, and the module AOI ran ahead of sequence intent every scan at exactly one scan of latency; [evidence](AllenBradley/Evidence/AB_S16_EXECUTION_EVIDENCE_2026-09-06.md). That evidence only became repeatable after the executor was fixed to read the context in one request: a 25-request per-member sweep tore against the 10 ms task exactly as S9's rule predicts, and made phases pass on one run and fail on the next. Still owed: registering S16 in the Phase 0 gate, an automated download on this workstation, and Core §6.1's answer on whether a HELD command may time out | the §6.1 lifecycle needs base structure Logix cannot express through composition, which would reopen AB §3.14 |
...
sequence execution, S12 public data contract and S16 command execution are all
settled, which closes
