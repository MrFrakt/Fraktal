...
| R4 gates | **PASS** | a fresh clone regenerates every fixture from an SDK seed, imports each at `0/0`, round-trips each canonically, passes a generated-versus-exported census, and clears Studio v33 **Verify Controller** at 0/0 on all nine legs; canonical form proved stable across seeds. Studio Verify needs a logged-in desktop and download is deliberately not automated — both itemised rather than glossed; evidence: [`AB_R4_REGENERATION_GATE_EVIDENCE_2026-09-06.md`](AllenBradley/Evidence/AB_R4_REGENERATION_GATE_EVIDENCE_2026-09-06.md) |
| R5 test execution | **PASS** | the disposable reference suite — two reference types with the module type instantiated **twice** — is regenerated by the gate, Verified `0/0`, allow-list audited, downloaded to the named bench and executed: nine machine-readable rows, all PASS on three consecutive runs, cross-talk zero. Echo is unavailable, so the CI path is the named isolated hardware with the download as an authorized manual step, recorded as a deliberate narrowing; evidence: [`AB_R5_REFERENCE_SUITE_EVIDENCE_2026-09-06.md`](AllenBradley/Evidence/AB_R5_REFERENCE_SUITE_EVIDENCE_2026-09-06.md) |
| R6 security | **PASS** for the declared read-only claim | zone/conduit `Z-CTRL`/`Z-ENG` with `C-EIP`/`C-ENG` and a declared **SL-T 1 / SL-C 0**; the External Access allow-list audit **run** against both downloaded fixtures, `Conforms: true`; the three-state client identity model with the transport's weakness stated; Core §14.2/§14.3 secret handling inherited; and an update lifecycle of regeneration gate plus recorded rollback. CIP Security, SL 2 and writes are explicitly **not** claimed; evidence: [`AB_R6_SECURITY_EVIDENCE.md`](AllenBradley/Evidence/AB_R6_SECURITY_EVIDENCE.md) |
...
**Published as of 2026-09-07.** S7 measured the shape; the press demo now
carries the content, emitted from the same declaration as its AOIs
([evidence](AllenBradley/Evidence/AB_MANIFEST_PUBLICATION_2026-09-07.md)). Two
rules are normative from that record. First, **the published key string is sized
to the declared maximum and its width is published in the header** as
`KeyLength`: a key that does not fit fails the build, because truncation
silently published two different paths under one name. Second, **acceptance is
not publication** — a manifest is only claimed once it has been read back out of
the imported project, since the first build imported with two warnings, zero
errors, and stored not one of its 208 strings.
...
**The named isolated-hardware CI path (S5, settled for the initial claim).**
Core §5.7's “runs in CI” is satisfied here by a **named isolated-hardware
runner**, not by Logix Echo. Echo is installed on no licensed Fraktal/AB
workstation and carries no activation feature in any local licence file, so the
Echo leg is unavailable rather than merely unused. The path is:

| Element | Value |
|---|---|
| Runner | the isolated Phase 0 bench, `1769-L24ER-QB1B/A`, firmware `33.014`, serial `7036B510` |
| Network | the bench's own segment; controller reached over EtherNet/IP for reads |
| Deployment conduit | Studio 5000 v33 over USB, `Backplane\16` — the engineering conduit of AB §11.2.1, never the gateway conduit |
| Suite | the disposable reference suite, regenerated from an SDK seed by the Phase 0 gate |
| Harvest | the fixed reference harness, hash-pinned `pylogix`, one row per test plus the five summary fields above |
| Results | machine-readable JSON, PASS/FAIL per row, values redacted |

**One step in that path is a manual, authorized operation, and this is a
deliberate narrowing rather than an omission.** The download is performed by a
person under current explicit authorization, with the controller identity
re-read immediately beforehand and all I/O disconnected. Everything on either
side of it — regeneration, import, Verify, canonical round trip, census, suite
execution, result harvesting — runs unattended and emits machine-readable
output.

This narrows Core §5.7 in a specific, stated way: the clause above says an
engineer manually pressing Run is not CI evidence, and that rule is kept. What
is manual here is **deployment**, not the test run: no human decides, observes
or reports a single suite row. The alternative would be an unattended automated
download to a physical controller, which this binding deliberately does not
build — AB §11.2.1 keeps the engineering conduit off the gateway and out of
permanent availability, and the Phase 0 toolchain contains no tool that can
download at all. **A binding that could deploy to a controller unattended would
be a worse binding**, and the decision record's D3 already took this shape of
trade once, when “parity” was narrowed to the shared contract suite rather than
an A/B rig to remove a hardware blocker without weakening the claim.

The consequence is accepted in writing: **Fraktal/AB does not claim
...
A deployment wanting the unattended form needs either CIP-Security-capable
hardware with a supported automated download path, or a licensed Echo
controller — and either is a new spike, not a configuration change.
...
| AB §3.10 | S7 | **PASS** | one bounded manifest fits: 43,728 bytes read completely and coherently in 293 ms at a 500-byte connection and 62 ms at 4000, with a ~32 ms header-only steady-state poll; the eight S7 capacity symbols are resolved at the measured sizes; [evidence](AllenBradley/Evidence/AB_S7_MANIFEST_EVIDENCE.md). **Content published 2026-09-07**: the press demo carries a generated manifest — 1 root, 4 modules, 142 fields, 5 operations, 206 localization keys, 12 reasons, `Valid`, not truncated — emitted from the same declaration as its AOIs, describing the declared graph once whichever rendition ran, and read back out of the imported project to prove it is stored rather than merely accepted; [evidence](AllenBradley/Evidence/AB_MANIFEST_PUBLICATION_2026-09-07.md). **Read from the controller 2026-09-08**: the whole 22,112-byte manifest comes off the bench in ten requests and 91 ms at a 4002-byte connection, coherent under the S7 revision protocol, with every published row equal to the declaration; [evidence](AllenBradley/Evidence/AB_MANIFEST_CONTROLLER_READ_2026-09-08.md) | settled — no per-root split is required; raising a capacity is a cost-curve calculation, not a new spike. Nameplates and optional profiles publish zero rows because this application declares none |
| AB §4.1 | S6 | OPEN | registry/manifest survive online change | commissioning gains a documented download step |
| AB §5.7 | S5 | **PASS** | the named isolated-hardware path is defined and executed: the disposable reference suite runs on the bench and emits nine machine-readable rows plus the five §5.7 summary fields, all passing on three consecutive runs. Echo is unavailable on every licensed workstation, so it is recorded as unavailable rather than unused, and the download is a declared authorized manual step — deployment, not the test run; [evidence](AllenBradley/Evidence/AB_R5_REFERENCE_SUITE_EVIDENCE_2026-09-06.md) | zero-touch CI to a physical controller is not claimed, and the full per-type harness grows with implementation |
...
| AB §2.5, §3.5 | S4 | OPEN | ST/LD plus native SFC routines, actions, transitions, settings and reset targets round-trip canonically. **The ladder half of the execution question is now closed**: an emitted RLL state machine walks a real application graph on the bench in step with the ST reference, which S4 never claimed - it proved RLL round-trips, not that one runs; [evidence](AllenBradley/Evidence/AB_LADDER_EXECUTION_PARITY_2026-09-07.md). Still open: the rest of the S4 construct matrix round-trip | disable native SFC; ST remains the reference sequence form |
...
| AB §2.8, §3.5, §4.1 | S11 | **PASS** | both generated forms walk one declared graph with identical traces and identical scan counts; module AOI runs unconditionally and before sequence intent; command/result latency is exactly one scan; simultaneous legs are per-branch; `SFR` re-entry repeats the run; [evidence](AllenBradley/Evidence/AB_S11_SEQUENCE_EXECUTION_EVIDENCE.md). **Extended to the third language:** the press AUTO graph is declared once and rendered in ST, native SFC and ladder, and all three walk byte-identical per-step entry-count vectors over a cycle, hold identically at the door-close step and stand down identically on abort - seventeen parity rows, three consecutive runs, no variance; [evidence](AllenBradley/Evidence/AB_LADDER_EXECUTION_PARITY_2026-09-07.md) | settled for ST, native SFC and ladder on the pinned v33 baseline, for the mechanics this graph uses; generated alternative branches and a sub-chain call are still not covered |
...
| AB §5.4 | S15 | **PASS**, narrowed | regeneration, SDK import with a gated log, export, canonical round trip, census and diagnostic capture all run unattended from a clean checkout across nine legs; Studio v33 **Verify Controller** runs automated but needs a logged-in desktop session, and download is deliberately not automated at all — on this workstation Studio additionally crashes when Download is invoked programmatically, three times across two mechanisms, with the controller untouched each time; [evidence](AllenBradley/Evidence/AB_R4_REGENERATION_GATE_EVIDENCE_2026-09-06.md) | headless Verify and unattended deployment remain unclaimed; neither is required by R4 |
...
**Every readiness gate R0–R6 now records PASS.** S1 transport, S2 AOI parameter
shape, S4 source fidelity, S5 test execution, S7 manifest read budget, S11
sequence execution, S12 public data contract, S15 the build gate and S16 command
execution are settled, which closes **R2**, supplies **R3** its resolved
capacities, and closes **R4** and **R5**. **R6 passes for the declared
read-only claim** and no further.

Three of those PASSes are bounded, and the bounds are the interesting part:
**S15** is narrowed — Studio Verify needs a logged-in desktop and download is
deliberately not automated; **S5** claims a named isolated-hardware path with an
authorized manual deployment step, not zero-touch CI; and **R6/S8** claims the
legacy zone-and-conduit posture with CIP Security, SL 2 and writes explicitly
unclaimed. **S9** still owes the mailbox matrix, which is owed only when writes
are enabled — so enabling writes is not a configuration flip but a return to
this register. S13/S14/S10 gate only the optional families/profiles they name.
