...
| Core §6.1, §6.2; AB §3.14 | S16 | OPEN — declared and generated, not executed | the Core §6.1 PLCopen handshake and the §6.2 mode chain execute on Logix through generated composition. Declared: the whole contract fits one `DINT`-only context UDT and two AOIs — module plus mode owner — using only types S12 proved, with the ordering rule self-checked in the fixture rather than documented. Generated, unit-tested and scope-fenced against runtime-base creep; import package and a nine-phase execution matrix are written but need the licensed v33 workstation; [declaration](AllenBradley/Evidence/AB_S16_COMMAND_HANDSHAKE_DECLARATION_2026-09-06.md). Still owed: SDK import, Studio v33 Verify, canonical round trip, and the executed matrix. Raised: Core §6.1 does not say whether a HELD command may time out, and the fixture freezes the clock while held | the §6.1 lifecycle needs base structure Logix cannot express through composition, which would reopen AB §3.14 |
