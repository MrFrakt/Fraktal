...
Usage
  python FraktalCore/PLC/TwinCAT/tools/plc_lint.py [root ...]   # default: the
                                             # TwinCAT tree this tool lives in
  python FraktalCore/PLC/TwinCAT/tools/plc_lint.py --quiet      # only failures
  python FraktalCore/PLC/TwinCAT/tools/plc_lint.py --profile 4024
                                             # legacy build: no 4026-only feature
...
# This tool ships inside the binding it lints, so both roots are anchored to the
# file rather than to the working directory: `plc_lint.py` with no arguments
# lints its own tree from anywhere, and rule E1 still finds the HMI enums it
# compares against. cwd-relative defaults silently scanned nothing (exit 2) or,
# worse for E1, skipped the comparison and reported clean.
TWINCAT_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_ROOTS = [TWINCAT_ROOT]
...
def lint_repository(roots: list[Path]) -> list[Finding]:
    ...
    dart_root = REPO_ROOT / "FraktalCore/HMI/lib/domain"
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("roots", nargs="*", type=Path, default=None,
                        help="paths to scan (default: the TwinCAT tree "
                             "this tool ships in)")
