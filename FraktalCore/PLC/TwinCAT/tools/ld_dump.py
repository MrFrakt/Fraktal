...
that catches those, and it costs one command:
    ...
    python FraktalCore/PLC/TwinCAT/tools/ld_dump.py <file.TcPOU>

`sfc_dump.py` is the same idea for an `<SFC>` chart.
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print(__doc__)
        print("usage: python .../TwinCAT/tools/ld_dump.py <file.TcPOU> [--no-ids]")
        return 2
