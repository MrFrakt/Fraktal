...
class _FieldbusTreeState extends State<FieldbusTree> {
  ...
  void initState() {
    super.initState();
    // The fieldbus I/O tree is drill-down data (Core §3.13): the direct OPC UA
    // transport reads it only while this page is mounted, keeping ~thousands of
    // channel-state nodes off the always-on snapshot. A no-op on other transports.
    widget.app.repo.setFieldbusViewActive(true);
  }

  @override
  void dispose() {
    widget.app.repo.setFieldbusViewActive(false);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (roots.isEmpty) {
      // A discovered-but-not-yet-populated topology (direct OPC UA/ADS reads it
      // on demand when this page opens) shows a loading indicator; a transport
      // with no fieldbus at all shows the empty state.
      if (widget.app.repo.fieldbusExpected) {
        return const Center(
