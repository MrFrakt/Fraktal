...
class LocalizationController extends ChangeNotifier {
  ...
  /// Administrator-owned catalog changes that can move between HMI
  /// installations. Language selection is intentionally absent because it is
  /// part of the local connection/bootstrap settings.
  Map<String, Object?> exportPortableOverrides() => {
        'schemaVersion': 1,
        'standard': _copyOverrides(_standardOverrides),
        'project': _copyOverrides(_projectOverrides),
      };

  Future<void> importPortableOverrides(Object? source,
      {bool merge = false}) async {
    if (source is! Map || source['schemaVersion'] != 1) {
      throw const FormatException('Invalid localization profile');
    }
    final standard = _parseOverrides(source['standard']);
    final project = _parseOverrides(source['project']);
    if (!merge) {
      _standardOverrides.clear();
      _projectOverrides.clear();
    }
    _mergeOverrides(_standardOverrides, standard);
    _mergeOverrides(_projectOverrides, project);
    await _persist();
    notifyListeners();
  }
  ...
  static Map<String, Map<String, String>> _parseOverrides(Object? source) {
    if (source is! Map) {
      throw const FormatException('Invalid localization catalog map');
    }
    final result = <String, Map<String, String>>{};
    for (final entry in source.entries) {
      if (entry.key is! String || entry.value is! Map) {
        throw const FormatException('Invalid localization catalog');
      }
      final language = entry.key as String;
      if (language.isEmpty || language.length > 20) {
        throw const FormatException('Invalid localization language');
      }
      final values = <String, String>{};
      for (final value in (entry.value as Map).entries) {
        if (value.key is! String || value.value is! String) {
          throw const FormatException('Invalid localized text');
        }
        final key = value.key as String;
        final text = value.value as String;
        if (key.isEmpty || key.length > 512 || text.length > 16000) {
          throw const FormatException('Invalid localized text size');
        }
        values[key] = text;
      }
  ...
  static Map<String, Object?> _copyOverrides(
          Map<String, Map<String, String>> source) =>
      {
        for (final language in source.entries)
          language.key: Map<String, String>.of(language.value),
      };

  static void _mergeOverrides(
    Map<String, Map<String, String>> target,
    Map<String, Map<String, String>> source,
  ) {
    for (final language in source.entries) {
      target.putIfAbsent(language.key, () => {}).addAll(language.value);
    }
  }
