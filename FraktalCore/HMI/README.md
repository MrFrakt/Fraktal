...
- Windows: native open62541 1.4.12 client through a small C ABI and a dedicated Dart worker isolate, or the WebSocket gateway client.
- Linux/Android native HMI: the WebSocket gateway client is implemented; the same direct OPC UA C ABI is source-compatible where its runner packages the native library.
...
The gateway builds as a headless Windows or Linux package from `gateway/`. It
can also serve the exact compiled Web HMI from the same loopback listener, so
...
The built-in simulation is selectable in the wizard. The production endpoint
on Windows/Linux/Android is an authenticated `wss://` Fraktal gateway. A Web
release opened from the packaged gateway automatically derives same-origin
`ws://.../fraktal` or `wss://.../fraktal`; browsers cannot open raw OPC UA TCP. A
Windows `opc.tcp://host:port` connection remains available as a conspicuously
logged Anonymous/None exception for local commissioning, troubleshooting, or a
physically isolated PLC. For same-host commissioning, start the packaged
service explicitly in `commissioning-anonymous` and use
`http://127.0.0.1:8080/`. Chrome debug keeps the explicit
`ws://127.0.0.1:8080/fraktal` development endpoint. Remote clients use an
authenticated TLS reverse proxy as described in `gateway/DEPLOYMENT.md`.

Native WSS deployments can supply a device mTLS certificate/key, optional plant
Web CA, or a bearer token through the protected `FRAKTAL_WSS_*` /
`FRAKTAL_GATEWAY_BEARER_TOKEN` process environment. Credential material is
accepted only for `wss://` and is never persisted in connection settings.
...
Automated CA enrollment/renewal UI, bundled reverse-proxy authentication,
Linux/Android native HMI runner packaging, and long-horizon historian storage
remain deployment work. The gateway already accepts provisioned OPC UA client
certificate/key/trust material, serves the Web HMI, and native clients can use
`ws://`/`wss://`. OEE
