...
void main() {
  test('createRepository skips the TCP preflight for ads:// endpoints',
      () async {
    ...
    await expectLater(
      createRepository(
          const ConnectionSettings(endpoint: 'ads://127.0.0.1:851')),
      throwsA(isA<UnsupportedError>()),
    );
  });

  test('ads endpoint with a non-6-octet AmsNetId is rejected clearly',
      () async {
    await expectLater(
      ...
      throwsA(isA<UnsupportedError>()
          .having((e) => e.message, 'message', contains('AmsNetId'))),
    );
  });
